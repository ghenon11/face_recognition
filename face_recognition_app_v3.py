...
class FaceRecognitionApp(ctk.CTk):
    ...
    def compare_faces(self):
        ...
        # Load known face encoding (before starting ProcessPoolExecutor)
        known_image = face_recognition.load_image_file(self.selected_image_path)
        known_encodings = face_recognition.face_encodings(known_image)
        ...
        # Convert known encodings to lists (picklable format)
        known_encodings_list = [enc.tolist() for enc in known_encodings]

        with concurrent.futures.ProcessPoolExecutor(max_workers=config.WORKERS) as executor:  #need processpool instead of threadpool as calling external C++ library
            futures = {executor.submit(process_image, item, known_encodings_list): item for item in items}
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if result:  # If a match was found
                        ...
                        logging.info(f"Image {file_path} processed")
                    with lock:
